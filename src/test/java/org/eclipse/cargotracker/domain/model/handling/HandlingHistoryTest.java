...
public class HandlingHistoryTest {
	...
	Voyage voyage = new Voyage.Builder(new VoyageNumber("X25"), SampleLocations.HONGKONG)
			...
			.addMovement(SampleLocations.DALLAS, LocalDateTime.now(), LocalDateTime.now()).build();
	HandlingEvent event1 = new HandlingEvent(cargo, DateUtil.toDateTime("2009-03-05","00:00"), LocalDateTime.now().plusDays(100),
			HandlingEvent.Type.LOAD, SampleLocations.SHANGHAI, voyage);
	HandlingEvent event1duplicate = new HandlingEvent(cargo, DateUtil.toDateTime("2009-03-05","00:00"), LocalDateTime.now().plusDays(200),
			HandlingEvent.Type.LOAD, SampleLocations.SHANGHAI, voyage);
	HandlingEvent event2 = new HandlingEvent(cargo, DateUtil.toDateTime("2009-03-10","00:00"), LocalDateTime.now().plusDays(150),
