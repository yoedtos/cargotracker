...
public class CargoRepositoryTest {
    ...
    public void testFindByCargoId() throws Exception {
        ...
        assertHandlingEvent(cargo, firstEvent, HandlingEvent.Type.RECEIVE, SampleLocations.HONGKONG, 100, 160, Voyage.NONE);

        HandlingEvent secondEvent = events.get(1);

        Voyage hongkongTonNewYork = new Voyage.Builder(
...
//        assertThat(event.getRegistrationTime()).isEqualTo(expectedRegistrationTime);
    ...
    private void assertLegCount(TrackingId trackingId, int expected) {
        ...
        // var count = this.entityManager.createQuery("select count(e) from Cargo c join c.itinerary.legs e where c.trackingId = :id")
...
//        var count = this.entityManager.createNativeQuery("select count(*) from Leg where cargo_id = ?1")
